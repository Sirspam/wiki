...
:::warning ВНИМАНИЕ Установка BMBF и модификация игры отключит возможность играть в официальный мультиплеер, просмотр и загрузку очков на официальную таблицу лидеров. If you would like to play modded multiplayer, you need the mods, `Beat Together` and `MultiQuestensions`, which allows for cross-play between PC and Quest and for custom songs to be used. These mods should automatically be installed when setting up BMBF.

Для того чтобы получить очки мастерства (Performance Points - PP) за определённые песни и попадать в списки лидеров на пользовательских песнях, вам нужен мод [ScoreSaber](https://new.scoresaber.com/quest). [Эта ссылка](https://new.scoresaber.com/quest) перенаправит вас на страницу его установки. ScoreSaber не заменяет официальные таблицы лидеров, а лишь добавляет их к пользовательским песням.

**Примечание:** Проверьте канал с обновленями в [ScoreSaber Discord](https://discord.gg/WpuDMwU), чтобы узнать, доступен ли мод для текущей версии. :::
