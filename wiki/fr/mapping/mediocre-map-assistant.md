...
_Informations essentielles pour démarrer avec Mediocre Map Assistant 2._
...
**Si vous avez la version de Quest -OU - vous n'avez pas installé Beat Saber sur votre ordinateur:**
...
* Alternativement, vous pouvez créer deux dossiers appelés `CustomLevels` et `CustomWIPLevels` puis y fournir leur chemin d'accès. Note that the usual Linux paths need to be prefixed with `Z:`.
...
::: warning
Les caractères spéciaux dans les langues telles que le japonais (日本語/にほんご), le Kaomoji (٩(◕‿◕｡)۶), le chinois (汉语/漢語), l'arabe (اَلْعَرَبِيَّةُ‎) et les accents (Ä/é/ó) ne sont pas entièrement pris en charge par BeatSaver.
Utiliser ces caractères dans les métadonnées ou dans les signets de la map peut causer des problèmes.
:::
